...
export function CreatePin({ onBack }: Props) {
  ...
  useEffect(() => {
    if (step !== 'preview' || !mapsReady || !svRef.current || pinLat === null || pinLng === null) return

    const sv = new google.maps.StreetViewService()
    sv.getPanorama({ location: { lat: pinLat, lng: pinLng }, radius: 200 }, (data, status) => {
      if (status === google.maps.StreetViewStatus.OK && svRef.current) {
        svPanoRef.current = new google.maps.StreetViewPanorama(svRef.current, {
  ...
  const shareUrl = pinLat !== null && pinLng !== null
  ...
  return (
    <div className="create-pin-page">
        ...
        <div className="step-dots">
          {(['pick', 'preview', 'share'] as Step[]).map((s, i) => (
            <div key={s} className={`step-dot ${step === s ? 'active' : ''} ${
              (['pick', 'preview', 'share'] as Step[]).indexOf(step) > i ? 'done' : ''
            }`} />
          ))}
        </div>
      </header>
      ...
      {step === 'pick' && (
        <div className="create-body">
          <div className="create-map-wrap">
            ...
            {pinLat === null && (
              <div className="map-prompt mono">Click anywhere on the map to drop your pin</div>
            )}
          </div>
          <div className="create-sidebar">
            <div className="sidebar-inner">
              ...
              <p className="sidebar-body">
                Click anywhere on the map to place your mystery location. Friends will see Street View and have to guess where it is.
              </p>
      ...
      {step === 'preview' && pinLat !== null && pinLng !== null && (
        <div className="create-body">
          <div className="create-map-wrap">
            <div ref={svRef} style={{ width: '100%', height: '100%', background: '#1a1a1a' }} />
            {!mapsReady && (
              <div className="sv-loading">
                <span style={{ fontSize: 28 }}>🌍</span>
                <span className="mono" style={{ fontSize: 12, color: '#888' }}>Loading…</span>
              </div>
            )}
          </div>
          <div className="create-sidebar">
            <div className="sidebar-inner">
              ...
              <p className="sidebar-body">
                This is what your friends will see. Look good? Add an optional hint, then generate the link.
              </p>
              ...
              <div className="sidebar-actions">
                <button className="btn-secondary" onClick={() => setStep('pick')}>← Repin</button>
                <button className="btn-primary mono" onClick={() => setStep('share')}>
                  Get link →
                </button>
              </div>
            </div>
        ...
        <div className="share-screen">
          <div className="share-card">
            ...
            <p className="share-body">
              Send this link to anyone. They'll see Street View and have to guess your exact location.
              The coordinates are hidden in the URL.
            </p>
            ...
            <div className="share-url-box mono" onClick={handleCopy} title="Click to copy">
              <span className="share-url-text">{shareUrl}</span>
              <span className="copy-badge">{copied ? '✓ Copied' : 'Copy'}</span>
            </div>

            <div className="share-actions">
              <button className="btn-secondary" onClick={() => setStep('preview')}>← Edit</button>
              <button className="btn-primary mono" onClick={() => {
            ...
            <button className="btn-text" onClick={onBack}>Back to game</button>
          </div>
        </div>
      )}
    </div>
