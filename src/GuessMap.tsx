...
export function GuessMap({ onGuess, guessLat, guessLng, revealLat, revealLng, locked }: Props) {
  ...
  useEffect(() => {
    ...
    mapRef.current.getContainer().style.cursor = locked ? 'default' : 'crosshair'
  }, [locked])
  ...
  useEffect(() => {
    const map = mapRef.current
    if (!map || !locked || revealLat === undefined || revealLng === undefined) return
    ...
    lineRef.current = L.polyline([[guessLat, guessLng], [revealLat, revealLng]], {
      color: '#666',
      weight: 1.5,
      dashArray: '5,5',
    }).addTo(map)

    map.fitBounds([[guessLat, guessLng], [revealLat, revealLng]], { padding: [40, 40] })
  ...
  useEffect(() => {
    ...
    if (correctMarkerRef.current) { map.removeLayer(correctMarkerRef.current); correctMarkerRef.current = null }
